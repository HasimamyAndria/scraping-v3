...
if __name__ == '__main__':
    ...
    history_filename = f'{os.environ.get("HISTORY_FOLDER")}/social-scraping-log.txt'
    # Enregistrement des logs des erreurs
    error_log_filename = f'{os.environ.get("ERROR_LOG_FOLDER")}/social-scraping-error-log.txt'
    
    now = datetime.now()
    if os.path.exists(history_filename):
        with open(history_filename, 'a', encoding='utf-8') as file:
            file.write("Démarrage scrap social: " +
    ...
    try:
        ...
        now = datetime.now()
        error_message = f"  ===>  Fin scrap social WITH ERRORS: {now.strftime('%d/%m/%Y %H:%M:%S')} - {str(e)}\n"
        with open(history_filename, 'a', encoding='utf-8') as file:
